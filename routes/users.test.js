...
describe("PATCH /users/:username", () => {
  ...
  test("unauth for different user and non-admin", async function () {
    const resp = await request(app)
        .patch(`/users/u2`)
        .send({
          firstName: "New",
  ...
  test("works: set new password by that user", async function () {
    const resp = await request(app)
        .patch(`/users/u1`)
        .send({
...
describe("DELETE /users/:username", function () {
  ...
  test("unauth for diff user and non-admin", async function () {
    const resp = await request(app)
        .delete(`/users/u2`)
        .set("authorization", `Bearer ${u1Token}`);
    expect(resp.statusCode).toEqual(401);
